...
仓库根目录的 `Dockerfile` 会构建前端静态资源，并由 FastAPI 与 API 一起在
`8000` 端口提供服务。镜像运行时需要在 SAE 环境变量中配置 DashScope/Qwen
密钥；不要将 `.env` 文件或密钥复制进镜像。

任务文件默认写入 `SCIDATA_RUNTIME_DIR`（容器默认 `/data/runtime`）。未挂载
持久化存储时，应用重建或实例替换后历史任务和上传文件会丢失，因此演示期间
请保持单实例并避免重启；生产部署应将该目录挂载到持久化存储。
